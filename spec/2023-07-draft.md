...
The `data` directory may contain an `invalid` directory containing testcases that must be rejected by validation.

Unlike `data/sample` and `data/secret`, there does not have to be a `bad-testcase.ans` file for every `bad-testcase.in`. 
Every `bad-testcase.in` without a matching `bad-testcase.ans` must be rejected by at least one input validator.

If `bad-testcase.ans` exists then `bad-testcase.in` must exist and must _pass_ input validation.
If `bad-testcase.out` exists, then `bad-testcase.ans` must exist, and the `.out`-file must be rejected by the output validator. 

<span class="not-icpc">
The directory `invalid` can organised into a tree-like structure similar to `secret`, and contain flags in `testdata.yaml` files that are passed to the validators.
</span>
