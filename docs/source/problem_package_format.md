...
This is the **legacy** version of the (Kattis) problem package format, roughly representing the state of Summer 2023 and aritrarily here versioned as `0.9.0` to facilitate referencing.

The present document avoids the distinction between ICPC and Kattis version of the specification, 
nor does it mark the deprecation or not-implemented status of various parts.
See the [overview page](index.md) for links to other versions of this document that keep those distinctions visible.
...
validators, graders and generators.
...
`sample` and `secret`. These two groups may be further split into subgroups as desired.
...
provided in 
`input_validators/`. Input validators can be specified as
